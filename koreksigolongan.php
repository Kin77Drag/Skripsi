...
<body>
			...
			if (isset($_GET['Golongan'])){
				$Golongan = filter_var($_GET['Golongan'], FILTER_SANITIZE_STRING);
				$sql = "SELECT `Golongan`, `Pangkat` FROM `golongan` WHERE Golongan='" . $Golongan . "'";
				$q = mysqli_query($koneksi, $sql);
				$r = mysqli_fetch_array($q);
			}
			?>
		<div class="container">
		<h2>Koreksi Tabel Golongan</h2>
		<form method="post">
		<div class="form-group row">
			<label for="Golongan" class="col-4 col-form-label">Nama Golongan</label> 
			<div class="col-8">
			<div class="form-group row">
                    <input id="Golongan" name="Golongan" type="text" class="form-control" value="<?php echo isset($r['Golongan']) ? $r['Golongan'] : ''; ?>" readonly>
            </div>
			</div>
		</div> 
		<div class="form-group row">
		...
		<?php
			if (isset($_POST['submit'])){
				...
				$sql="UPDATE `golongan` SET `Pangkat`='".$Pangkat."' WHERE Golongan='".$Golongan."'";
